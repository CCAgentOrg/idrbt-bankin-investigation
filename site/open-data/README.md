...
The domain lists from this investigation feed into the **[bank-in-domains](https://github.com/CCAgentOrg/bank-in-domains)** project — a daily automated audit of `*.bank.in` and parallel Indian financial TLDs. Every day at 02:30 UTC, it discovers new subdomains and probes each one for DNS resolution, HTTPS reachability, HTTP status codes, and page titles. Results are published as CSV, JSONL, Parquet, and SQLite.

This data enables public uptime monitors like **[uptime.cashlessconsumer.in](https://uptime.cashlessconsumer.in)** and independent verification by any researcher, journalist, or concerned citizen.
